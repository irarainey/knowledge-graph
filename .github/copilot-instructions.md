...
- **Graph renderer** (`frontend/graph-renderer/`) — Vue 3 / TypeScript SPA using pnpm. Renders the knowledge graph from the static `data/aircraft-knowledge-graph.json` export, overlaid with the `data/sdlc-knowledge-graph.json` engineering/SDLC graph (no backend calls).
...
- **Neo4j** — Graph database running as a Docker container. Stores the operational aircraft graph plus the imported SDLC/engineering overlay (two domains joined by cross-domain edges) as nodes and relationships queried with Cypher.
